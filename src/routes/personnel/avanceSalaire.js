...
const { AvanceSalaire, Salaire, PaiementSalaire, Personnel, Poste } =require('../../db/sequelize');
const {protrctionRoot, authorise} = require('../../middleware/protectRoot');
const { where } = require('sequelize');

// 1. CREATE - Enregistrer un paiement de salaire
addASalaire = (app) => {
    app.post('/addASalaire', protrctionRoot, authorise('admin', 'comptable'), async (req, res) => {
        try {
            //verification
            ...
            if(som === salaire.montant_net ){
                ...
                const nouveauASalaire = await AvanceSalaire.create(req.body);
...
deleteASalaire = (app) => {
    app.delete('/deleteASalaire/:id', protrctionRoot, authorise('admin', 'comptable'), async (req, res) => {
        try {
            const result = await AvanceSalaire.update(
                { is_active: false },
                { where: { id_avance: req.params.id } }
            );
            
            if (result[0] > 0) {
                res.redirect('/allSalaire?mag=Avance Salaire supprimer avec seccess&tc=alert-success');
            } else {     
